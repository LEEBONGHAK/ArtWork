...
# 배포 peer0.org1.artwork.com  -> dev-ArtWork 인도서 피어 컨테이너가 생성, 커미터 피어 couchdb1 myart_papercontract 테이블이생성
docker exec cli peer chaincode instantiate -n ArtWork -v 0.9 -C myart -c '{"Args":[]}' -P 'OR ("Org1MSP.member","Org2MSP.member")' 
